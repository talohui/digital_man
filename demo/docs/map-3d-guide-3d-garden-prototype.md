...
阶段七十八 B 对 `/map-3d-guide-c?debugGarden=1` 的编辑器做可用性收敛，重点解决旧草稿残留、zone 色块过重和使用路径不清晰的问题。

### 区域显示

vegetation zone 和 keepout zone 默认改为“边框 + 顶点”表达：

- vegetation zone：青绿色边框。
- keepout zone：红棕色边框，尽量使用虚线配置。
- 非选中区域只保留接近透明的极低填充。
- 选中区域才保留约 0.08 的弱填充，便于辨认当前编辑对象。

这样可以避免大面积色块盖住腾讯底图、金色路线、POI、当前位置和 3D 树群。
...
新增“清空本地草稿”，并强化“重置为默认航拍参考布局”。两者都会清理 debugGarden 相关 localStorage，恢复源码内置的默认航拍参考树群，不需要用户手动打开浏览器 DevTools 删除数据。

### 帮助入口

编辑器面板新增“使用说明 / 帮助”折叠区，简要说明：

1. 先画 keepout 禁放区。
2. 再画 vegetation 放树区。
3. 生成预览点并检查是否避开路线、广场、建筑和站点。
4. 应用为 GLB 树群。
5. 保存到本地并导出完整 TS 配置。

完整说明写入 `docs/map-3d-guide-garden-editor-usage.md`，作为人工校准 C 版园林资产的操作手册。
