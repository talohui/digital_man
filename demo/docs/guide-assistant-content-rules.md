...
路线到达卡通过 `getPoiArrivalSummary(poiId)` 获取当前站点的短摘要，正式 POI 在 `poiGuideMetadata.ts` 中维护适合移动端卡片阅读的到达文案。页面不再使用“适合听建筑与灵山历史渊源”的统一模板。

新 POI 的回退顺序为：现有详情正文的第一句、现有详情简介、`guideSpots` 简介、景点目录基础提示。页面再根据真实 GLB 注册补充“查看图文介绍与 3D 模型”或“查看景点介绍与游览建议”的结尾。
