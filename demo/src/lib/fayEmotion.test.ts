...
test('uses a smile for positive and neutral user text', () => {
  assert.equal(getReplyEmotionStateForUserText('今天玩得很开心'), 'happy')
  assert.equal(getReplyEmotionStateForUserText('灵山大佛有多高'), 'happy')
})
...
test('keeps neutral user questions smiling instead of using assistant emotion', () => {
  assert.equal(getReplyEmotionStateForUserText('灵山大佛有多高'), 'happy')
  assert.equal(
    resolveReplyRobotState('happy', 'speaking', createFayEmotionCue({ Data: { Sentiment: 2 } }, '')),
    'happy'
  )
  assert.equal(resolveReplyRobotState('happy', 'speaking', createFayEmotionCue({}, '')), 'happy')
})
