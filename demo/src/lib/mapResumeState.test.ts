...
test('browse-map exit does not replace the last route and stop', () => {
  const routeUrl = '/map-3d-guide-c/route/family?stage=arrived&stop=3&presentation=scenic3d'
  saveMapResumeState({ url: routeUrl, presentation: 'scenic3d' })

  const saved = saveMapResumeState({ url: '/map-3d-guide-c?presentation=scenic3d', presentation: 'scenic3d' })

  assert.equal(saved?.url, routeUrl)
  assert.equal(getMapResumeUrl(), routeUrl)
})
