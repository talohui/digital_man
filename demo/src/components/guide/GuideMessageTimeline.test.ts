...
test('renders assistant Markdown instead of exposing raw emphasis markers', () => {
  assert.match(source, /import ChatMarkdown from '\.\.\/ChatMarkdown'/)
  assert.match(source, /message\.role === 'assistant'/)
  assert.match(source, /<ChatMarkdown content=\{message\.text\} \/>/)
})
