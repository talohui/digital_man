...
const robotExpressionCues: Record<string, { icon: string; label: string }> = {
  normal: { icon: '🙂', label: '小灵正在陪伴' },
  listening: { icon: '👂', label: '小灵正在倾听' },
  thinking: { icon: '💭', label: '小灵正在思考' },
  speaking: { icon: '💬', label: '小灵正在讲解' },
  happy: { icon: '😊', label: '小灵很高兴' },
  comfort: { icon: '🤍', label: '小灵温柔陪伴' }
}
...
export function XiaolingConversationSurface({
  ...
  const expressionCue = robotExpressionCues[runtime.robotState] ?? robotExpressionCues.normal
    ...
    >
      ...
      <header className="immersive-guide__topbar xiaoling-conversation__topbar">
        ...
        <div className="xiaoling-conversation__heading">
          <span><i aria-hidden="true">{expressionCue.icon}</i>{subtitle}</span>
          <strong>{title}</strong>
        </div>
      ...
      <div className="xiaoling-conversation__hud" aria-label={`小灵状态：${expressionCue.label}；当前位置：${locationLabel}`}>
        <div className={`immersive-guide__status xiaoling-conversation__status xiaoling-conversation__status--${runtime.robotState}`}>
          <span aria-hidden="true" />
          <strong>{expressionCue.label}</strong>
        </div>
        <div className="xiaoling-conversation__location" aria-label={`当前位置：${locationLabel}`}>
          <span aria-hidden="true">⌖</span>
          <div><small>当前位置</small><strong>{locationLabel}</strong></div>
        </div>
      </div>
        ...
        <form className="chat-card__composer-stack" onSubmit={submit}>
          <div className="chat-card__composer">
            ...
            <textarea
              ...
              aria-label={`输入问题，${expressionCue.label}`}
            />
