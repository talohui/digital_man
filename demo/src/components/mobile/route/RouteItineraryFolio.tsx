...
import { MapFolioMetrics, MapFolioSurface } from '../../map/MapFolioPrimitives'
...
function RouteItineraryFolio({
  ...
  return (
    <MapFolioSurface className="route-itinerary-folio" aria-labelledby={`route-itinerary-${route.id}-title`}>
      ...
      <MapFolioMetrics
        className="route-itinerary-folio__facts"
        ariaLabel="路线信息"
        items={[
          { label: '预计用时', value: formatDuration(route.estimatedMinutes) },
          { label: '沿途站点', value: `${route.stops.length} 站` },
          { label: '步行强度', value: route.guideRoute.walkIntensity }
        ]}
      />
