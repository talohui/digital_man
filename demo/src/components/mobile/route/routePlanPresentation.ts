...
/**
 * The route-planning prompt asks Xiaoling to name one of the published routes in
 * its first sentence. The UI only acts on those three catalog names, never on a
 * free-form destination invented by the model.
 */
export function getRouteIdFromXiaolingReply(reply: string): string | null {
  const normalized = reply.replace(/\s/g, '')
  if (normalized.includes('历史文化爱好者路线')) return 'historical_culture'
  if (normalized.includes('自然风光爱好者路线')) return 'natural_scenery'
  if (normalized.includes('亲子家庭路线')) return 'family'
  return null
}
