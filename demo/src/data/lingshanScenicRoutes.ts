...
const ROUTE_CAMERA_PRESETS: Record<string, string> = {
  ...
  natural_scenery: 'guideFollow',
  family: 'guideFollow'
}

const LEGACY_ROUTE_ID_ALIASES: Record<string, string> = {
  prayer_meditation: 'natural_scenery'
}

export function normalizeScenicRouteId(routeId?: string | null) {
  if (!routeId) return routeId
  return LEGACY_ROUTE_ID_ALIASES[routeId] ?? routeId
}
...
export function getScenicRouteConfig(routeId?: string | null) {
  ...
  const canonicalRouteId = normalizeScenicRouteId(routeId)
  return scenicRouteConfigs.find((route) => route.id === canonicalRouteId) ?? scenicRouteConfigs[0]
}
...
export function getScenicRouteById(routeId?: string | null) {
  ...
  const canonicalRouteId = normalizeScenicRouteId(routeId)
  return scenicRouteConfigs.find((route) => route.id === canonicalRouteId)
}
