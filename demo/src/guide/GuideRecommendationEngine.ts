...
const profiles: RouteProfile[] = [
  ...
  { routeId: 'natural_scenery', interests: ['nature', 'relax', 'photo'], pace: 'relaxed', suitableFor: ['solo', 'elderly', 'couple', 'friends'] },
  { routeId: 'family', interests: ['family', 'relax', 'photo', 'nature'], pace: 'relaxed', suitableFor: ['family', 'elderly', 'friends'] }
]

const reasons: Record<ScenicRouteId, string> = {
  historical_culture: '佛教历史与建筑内容最集中，适合深入了解灵山人文主线',
  natural_scenery: '串联太湖视野、园林景观与禅意空间，适合自然风光爱好者慢游拍照',
...
export function recommendScenicRoutes(preferences: UserTravelPreferences): RouteRecommendationItem[] {
  const scored = profiles.map((profile) => {
    ...
    if (preferences.availableMinutes && preferences.availableMinutes <= 150 && profile.routeId === 'family') score += 7
