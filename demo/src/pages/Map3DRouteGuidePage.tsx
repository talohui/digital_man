...
const ROUTE_TAB_LABELS: Record<string, string> = {
  historical_culture: '历史文化爱好者路线',
  natural_scenery: '自然风光爱好者路线',
  family: '亲子家庭路线'
}

const ROUTE_PREVIEW_TAB_IDS = ['historical_culture', 'natural_scenery', 'family']

const ROUTE_PREVIEW_SUMMARIES: Record<string, string> = {
  historical_culture: '覆盖灵山代表人文主线，听佛教历史与建筑故事。',
  natural_scenery: '串联太湖视野、园林景观与禅意空间，适合慢游和拍照。',
...
function RouteActiveCard({
  ...
  return (
    <section className="map-route-tour-card map-route-tour-card--active" aria-label="路线进行中">
      <div className="map-route-tour-card__tip">下一站为{nextStop?.name ?? '下一站'}。需要实时转向时请以地图定位和现场标识为准。</div>
      ...
      <p className="map-route-tour-meta">下一站游览建议 · {stayLabel}</p>
      ...
      <div className="map-route-tour-actions">
        ...
        <button type="button" onClick={onSkipNavigation} aria-label="跳过导航，直接到站">
          跳过导航，<br aria-hidden="true" />直接到站
        </button>
  ...
  return (
    <section className="map-route-tour-card map-route-tour-card--arrived" aria-label="已到达景点">
      ...
      <div className="map-route-tour-actions map-route-tour-actions--arrived">
        <button type="button" className="map-route-tour-primary" onClick={onListenGuide}>
          听小灵讲解
        </button>
...
function RouteTourMobileOverlay({
  ...
  const currentStopPoiId = getStopPoiId(getRouteStopByIndex(route.id, currentStopIndex))
  ...
  }, [route.id, setActiveRouteId])

  useEffect(() => {
    if ((stage === 'active' || stage === 'arrived') && currentStopPoiId) {
      markStopVisited(currentStopPoiId)
    }
  }, [currentStopPoiId, markStopVisited, stage])
  ...
  const handleSkipNavigation = () => {
    ...
    const targetStopIndex = currentStopIndex + 1
    const targetStop = getRouteStopByIndex(route.id, targetStopIndex)
    setFeedbackText(`已跳过步行导航，进入${targetStop?.name ?? '下一站'}到站页`)
    goToRouteArrived(navigate, route.id, targetStopIndex, presentation)
  }
  ...
  }, [setLayerPanelOpen, simulatedNavigation])
  ...
  const handlePreviewGuide = (stop: ScenicRouteStop | undefined) => {
    ...
    const poiId = getStopPoiId(stop)
    if (poiId) {
      markStopListened(poiId)
    }
    setServiceOpen(false)
    openGlobalXiaoling({
      mode: 'route',
  ...
  const handleListenCurrentStop = () => {
    handlePreviewGuide(getRouteStopByIndex(route.id, currentStopIndex))
  }
  ...
  return <>
    ...
    >
      ...
      <RouteToolRail
        ...
        isLayerOpen={layerPanelOpen}
      />
      <div className="map-route-tour-bottom">
        {navigationBetaViewModel.shouldReplaceRouteSheet ? (
          ...
          <RouteArrivedCard
            ...
            onListenGuide={handleListenCurrentStop}
          ...
          <RouteActiveCard
            ...
            onNavigate={handleNavigateNext}
            onSkipNavigation={handleSkipNavigation}
            onPoiDetail={handlePoiDetail}
