...
export function NavigationPrototypeCard() {
  ...
  const firstInstruction = useMemo(() => route?.steps[0]?.instruction, [route])
  ...
  return (
    <section className={`navigation-prototype-card is-${status}`} aria-label="腾讯步行导航原型">
      ...
      {route && (status === 'navigating' || status === 'arrived') ? (
        <div className="navigation-prototype-card__details">
          ...
          <dl>
            ...
            <div>
              <dt>预计</dt>
              <dd>{formatDuration(route.durationMinutes)}</dd>
            </div>
            <div>
              <dt>路线点</dt>
          ...
          {firstInstruction ? <p className="navigation-prototype-card__instruction">首条指引：{firstInstruction}</p> : null}
          {status === 'navigating' ? (
            <button type="button" className="navigation-prototype-card__primary" onClick={simulateArrival}>
              模拟到达
            </button>
          ) : (
            <p className="navigation-prototype-card__arrived">已到达灵山大照壁（原型状态）</p>
          )}
        </div>
      ) : null}
