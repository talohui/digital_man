...
import { resolveRouteRequest, scoreRoute } from '../components/mobile/route/routePlanPresentation'
import {
...
function MobileRoutePlanPage() {
  ...
  const [planState, dispatch] = useReducer(
    routePlanReducer,
    {
      selectedRouteId: restoredSession?.selectedRouteId ?? activeRouteId,
  ...
  const mappableCandidateRoutes = useMemo(() => {
    ...
    return safeRoutes.length ? safeRoutes : buildLocalGuideRecommendations(selectedTags, guidePreferences)
  }, [candidateRoutes, guidePreferences, selectedTags])
  ...
  const submitRouteRequest = (requestText: string) => {
    ...
    const resolved = resolveRouteRequest(request)
    const currentState = useGuideStore.getState()
    const currentTags = currentState.selectedTags
    let preferencesChanged = false
    if (resolved.tags.length) {
      GUIDE_TAGS.forEach((tag) => {
        const shouldSelect = resolved.tags.includes(tag)
        if (currentTags.includes(tag) !== shouldSelect) {
          preferencesChanged = true
          toggleTag(tag)
        }
      })
    }
    Object.entries(resolved.preferences).forEach(([key, value]) => {
      const preferenceKey = key as GuidePreferenceKey
      if (value && currentState.guidePreferences[preferenceKey] !== value) {
        preferencesChanged = true
        setGuidePreference(preferenceKey, value)
      }
    })
    dispatch({
      type: 'SUBMIT_REQUEST',
      ...
      awaitRecommendations: preferencesChanged
    })
    if (!preferencesChanged && mappableCandidateRoutes[0]) {
      dispatch({
        type: 'SUGGEST_ROUTE',
        routeId: mappableCandidateRoutes[0].id,
        message: `小灵已首荐${mappableCandidateRoutes[0].name}`
      })
    }
  }

  const acceptSmartRoute = () => {
    const recommendedRoute = mappableCandidateRoutes[0]
          ...
          <section className="plan-v2-intro" aria-labelledby="plan-v2-title">
            ...
            <MountainGate variant="services" className="plan-v2-xiaoling-gate" delay={120}>
              <div className="plan-v2-xiaoling-note">
                <span className="plan-v2-xiaoling-note__avatar"><img src="/icons/lingshan-guide-avatar.png" alt="" /></span>
