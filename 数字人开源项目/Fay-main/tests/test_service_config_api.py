...
def test_session_verification_accepts_only_an_existing_fay_admin_session(tmp_path):
    client, _, _ = create_client(tmp_path)

    denied = client.post("/api/admin/service-config/verify-session")
    allowed = client.post(
        "/api/admin/service-config/verify-session",
        headers=login(client),
    )

    assert denied.status_code == 401
    assert allowed.status_code == 200
    assert allowed.get_json() == {"verified": True}
    assert allowed.headers["Cache-Control"] == "no-store"
