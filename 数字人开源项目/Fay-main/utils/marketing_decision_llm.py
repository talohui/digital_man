...
def validate_decision_payload(payload, decision_input=None):
    ...
    if isinstance(decision_input, dict):
        ...
        active_emergencies = decision_input.get("activeEmergencies")
        if isinstance(active_emergencies, list) and active_emergencies:
            if not any(card["type"] == "应急处置" for card in normalized["cards"]):
                raise ValueError("model decision is missing emergency card")
        consumption_signal = decision_input.get("consumptionSignal")
        if isinstance(consumption_signal, dict) and consumption_signal.get("available"):
            if not any(card["type"] == "消费转化" for card in normalized["cards"]):
                raise ValueError("model decision is missing consumption card")
...
def build_messages(decision_input):
    active_emergencies = (
        decision_input.get("activeEmergencies")
        if isinstance(decision_input, dict)
        else None
    )
    ...
    if isinstance(active_emergencies, list) and active_emergencies:
        emergency_instruction = (
            "输入含生效中的应急事件时，必须至少生成一张应急处置卡，并优先列出游客提醒、"
            "路线调整、数字人播报和现场处置；不得把营销动作置于应急处置之前。"
            "只能使用输入中已确认的事件事实，不得扩写伤情、位置、人数或处置结果。"
        )
    consumption_signal = (
        decision_input.get("consumptionSignal")
        if isinstance(decision_input, dict)
        else None
    )
    ...
    if isinstance(consumption_signal, dict) and consumption_signal.get("available"):
        consumption_instruction = (
            "输入含消费转化信号时，必须至少生成一张消费转化卡，并在证据中注明输入的来源标签。"
            "若 historicalBaseline 为 true，必须明确写为历史样本或推荐先验，绝不能表述为实时成交。"
        )
    schema = {
        "summary": "运营摘要",
        "cards": [
            {
                "title": "决策标题",
                "type": "应急处置/营销机会/服务优化/客流分流/知识库补全/技术优化/客群营销/消费转化",
                "priority": "高/中/低",
                "evidence": ["只能引用输入中的证据"],
                "reason": "原因",
                "actions": ["可执行动作"],
